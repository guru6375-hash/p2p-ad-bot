...
It lists your advertisements and reprices your existing **buy** advertisements for the PLN
pairs (PLN/USDT, PLN/USDC) from the competitor market, keeping every price under a hard
**cap_rate** ceiling. Sell advertisements are never touched, and no advertisement is ever
created.
...
**Binance** needs a key that may call the C2C Agent API (`/sapi/v1/c2c/agent/ads/*`). When an
edit names payment methods, the adapter resolves them against the account's own methods, so
the key must also be able to read them (`_resolve_trade_methods`). Binance displays the HMAC secret exactly once, behind
...
`python run.py verify-config`, then `python run.py pln-edits --scenario pln --dry-run` — it
reads every account's ads (changing nothing) and reports the per-account errors.
...
```jsonc
{
  ...
  "parser": {"enabled": true, "interval_minutes": 25, "cron": "*/25 * * * *"},
  "defaults": {"min_amount": "500", "max_amount": "100000",
               "payment_methods": ["BLIK"], "price_offset": "0"},
  "pairs": [
    {"pair": "PLN/USDT", "anchor": true,  "accounts": ["Binance#1", "Okx#1", "Bybit#1"]},
    {"pair": "PLN/USDC", "anchor": false, "linked_to": "PLN/USDT", "accounts": ["Binance#1", "Bybit#1"]}
  ]
}
```
...
`platforms.<name>.source` and per-platform `filters`. A non-anchor pair must declare
`linked_to`.
...
* `python run.py pln-edits` pushes these prices to the live **buy** ads of the PLN pairs
  through a size-1 producer/consumer queue (one edit pending at a time, each applied with
  `edit_ad`, which keeps the ad's own limits, amount and payment methods).

### Cron settings (`p2pbot/cron_config.py`)

Two values at the top of the file control the automatic buy-ad repricing:

```python
SCHEDULE_TIME = 25               # minutes between two runs of the pln-edits job
PAIRS = ["USDT", "USDC"]         # tickers (PLN/<ticker>) or full symbols like "PLN/USDT"
```

`python run.py bot` registers a `pln-edits` job that runs the edit queue every
`SCHEDULE_TIME` minutes for `PAIRS` only; `python run.py pln-edits` uses the same `PAIRS`.
A bad value (non-positive or non-integer minutes, an empty list, a non-PLN or duplicate
pair) stops the bot at start-up with a message naming the setting, and
`python run.py verify-config` checks both values and that every pair is in a blueprint.

### UAH rate (`/setrate`, `p2pbot/uah_config.py`)

`/setrate <RATE>` in Telegram reads the own ads of every configured account on Binance, OKX
and ByBit and reprices them through the same size-1 producer/consumer queue, as a **ladder
per account and pair**: the online **buy** ads, ranked by their current price, get `RATE`,
`RATE - STEP`, `RATE - 2·STEP`, … for `UAH/USDT` and the same ladder one STEP lower for
`UAH/USDC` (`STEP` per exchange). No cap is required for the UAH pairs; when one is stored
with `/setcap`, the top of that pair's ladder is clamped to it. (PLN pairs still need a cap:
without one they are never priced.) Sell and offline ads are left alone and listed as
problems.

```python
STEP = {"binance": "0.25", "okx": "0.01", "bybit": "0.01"}   # p2pbot/uah_config.py
```

`/setrate 45.00` on an account with 3 USDT + 3 USDC buy ads → Binance: USDT 45.00 / 44.75 /
44.50, USDC 44.75 / 44.50 / 44.25; ByBit: USDT 45.00 / 44.99 / 44.98, USDC 44.99 / 44.98 /
44.97. `/setrate 45.00 --dry` shows the same edits without sending them.

### The cap is absolute

`/setcap <PAIR> <RATE>` stores `cap_rate`. Every computed price is clamped to it before it
is sent to a venue — cap beats every other rule. A pair without a stored cap is **not
priced at all** (fail-closed), so `pln-edits` never edits its ads.
...
zero USDC ads for PLN). Those entries are **skipped per (pair, platform)** — every other
advertisement is still corrected — and reported as `skipped <n>` lines by `/rates` and as
`problem` lines by `pln-edits`.

### Optional `.env` switches

```dotenv
SCENARIO=pln                 # blueprint activated at startup (also settable with /scenario)
```
...
python run.py rates    --scenario pln      # print computed prices, change nothing
python run.py parser   --scenario pln      # one competitor-parser pass (--dry-run: print only)
python run.py pln-edits --scenario pln     # reprice the live PLN buy ads via a size-1 edit queue
...
`1` with `no active scenario; available: …`, and a `/setcap` for a pair is what lets it be
priced — until then it refuses rather than guessing a price.

Run `bot` under any supervisor (systemd, Windows Task Scheduler, `nssm`, Docker); it runs the
25-minute parser job. Schedule `python run.py pln-edits` to push the prices to the buy ads.

Exit codes: `0` success, `1` configuration/state fault (including `pln-edits` when problems
— missing cap, no market data, unlistable account — blocked *every* edit), `2` unexpected
runtime error.
...
```
/start /help                     usage
/setbase PLN/USDT 3.85           store base_rate
/setcap  PLN/USDT 3.90           store cap_rate (hard ceiling)
/setrate 45.00 [--dry]           reprice UAH buy ads as a STEP ladder (no cap needed)
/getads [PAIR|all] [--offline]   list your ads per account (default UAH/USDT + UAH/USDC)
/rates                           base/cap table + computed prices
/scenarios                       list blueprints
/scenario pln                    activate a blueprint
```
...
* **Binance** manages ads through the C2C Agent API (`/sapi/v1/c2c/agent/ads/*`). Updates are
  built for **buy** ads only (a sell ad is refused before any request) and write the ad's
  payment methods back as `identifier` entries, because `getDetailByNo` answers with the
  read shape only.
...
* **OKX** ignores the `userType` query parameter; merchant detection uses `creatorType` /
  ...
  private path `POST https://www.okx.com/api/v5/p2p/ad/update` answers
...
* **Binance/Bybit private ad management** is production-verified at the transport level: with
  placeholder credentials, `POST api.binance.com/sapi/v1/c2c/agent/ads/{update,listWithPagination,getDetailByNo}`
  returns `{"code":-2008,"msg":"Invalid Api-Key ID."}` and
  `POST api.bybit.com/v5/p2p/item/update` returns `{"ret_code":10003,"ret_msg":"API key is invalid."}`
...
* A venue that reports a failure for one account is isolated and reported per account.

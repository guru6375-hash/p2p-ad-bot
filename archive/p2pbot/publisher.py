...
* :class:`AdPublisher` — reads the live advertisements of every account
  ...
  sell ad; a stored ``cap_rate`` is re-asserted before a request is built.
...
class AdPublisher:
    ...
    Args:
        ...
        rates: rate store; a stored ``cap_rate`` is re-asserted for every edit.
    ...
    def edit_ad(
        ...
        *,
        ...
        ad without ``active=True``; a price above the pair's stored ``cap_rate`` (a fixed
        price is clamped to it, a floating ratio whose estimated price exceeds it is
        refused). Pairs without a stored cap are edited as asked.
        """
    ...
    def _edited_spec(
        ...
        *,
        ...
        cap = self.rates.cap(live.pair)
        if cap is not None and new_price > cap:
            if ratio is not None:
                raise ConfigError(
                    f"floating ratio {ratio} would price the ad at about {new_price:.4f}, above "
                    f"the {live.pair.symbol} cap {cap}"
                )
            _log.warning(
                "cap enforced on edit of %s %s: %s clamped to %s",
                live.account_id, live.adv_no, new_price, cap,
            )
            new_price = cap

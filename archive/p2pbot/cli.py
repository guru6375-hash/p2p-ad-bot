...
``run.py`` is the only entry point an operator needs::
    ...
    python run.py rates  --scenario pln     # print the computed prices
    python run.py pln-edits --dry-run       # reprice the live PLN buy ads
...
* A price is printed as ``PAIR PLATFORM price source [clamped]`` and an edit attempt as
...
from .cron_config import load_cron_config
...
def build_parser() -> argparse.ArgumentParser:
    ...
    _add_command(subparsers, "rates", "print the computed advertisement prices",
                 scenario=True)
    _add_command(subparsers, "pln-edits",
                 "reprice the live PLN ads through a size-1 producer/consumer edit queue",
                 scenario=True, dry_run=True)
...
def _dispatch(
    ...
    return _run_pln_edits(resolved, scenario, dry_run)
...
def _run_pln_edits(services: "BotServices", scenario: str | None, dry_run: bool) -> int:
    """``pln-edits``: one producer/consumer pass over ``cron_config.PAIRS``."""
    from .edit_queue import run_pln_edit_queue  # lazy, same reason as build_services

    blueprint = _select_scenario(services, scenario, align_job=False)
    cron = getattr(services, "cron", None)
    report = run_pln_edit_queue(
        ...
        dry_run=dry_run,
        pairs=None if cron is None else cron.pairs,
    )
    for edit in report.queued:
        print(f"queued    {edit.describe()}")
    for edit in report.unchanged:
        print(f"unchanged {edit.describe()}")
    for result in report.results:
        print(f"edited    {_format_result(result)}")
    for problem in report.problems:
        print(f"problem   {problem}")
    if not (report.queued or report.unchanged or report.problems):
        print("no PLN ads to edit")
    # 1 when problems blocked everything, like publish/tick
    return 1 if report.problems and not (report.results or report.unchanged) else 0
...
def _verify_config(settings: Settings) -> int:
    """``verify-config``: validate ``.env``, every blueprint, cron_config.py and uah_config.py."""
    problems: list[str] = []
    try:
    ...
    for path in blueprints:
        try:
            ...
            continue
        scenario_pairs.update(plan.pair.symbol for plan in blueprint.pairs)
        unknown = sorted(
            {
                account_id
    ...
    try:
        ...
        absent = [pair.symbol for pair in cron.pairs if pair.symbol not in scenario_pairs]
        if absent:
            problems.append(f"cron_config.PAIRS not in any blueprint: {', '.join(absent)}")
        print(f"ok cron_config.py: {cron.describe()}")
...
def _format_result(result: Any) -> str:
    """``ACCOUNT PAIR status price adv_no|error`` - one line per edit attempt."""

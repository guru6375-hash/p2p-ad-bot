...
def _publisher(
    ...
    *,
    ...
    publisher = AdPublisher(resolved, settings, ad_store, rate_store, dry_run)
...
def _editor(settings, *ads: OwnAd, store=None, rates=None, **adapter_kwargs):
    adapter = _FakeAdapter("binance", adv_no=None, own_ads=ads or (_live(),), **adapter_kwargs)
    return _publisher(settings, adapters={"binance": adapter}, store=store, rates=rates)
...
def test_edit_ad_changes_only_the_given_field_of_the_live_ad(settings, tmp_path: Path) -> None:
    ...
    publisher, adapters, _store, _rates = _editor(settings, store=store)
...
def test_edit_ad_overrides_limits_quantity_and_payment_methods(settings) -> None:
    ...
    publisher, adapters, _store, _rates = _editor(settings, ad)
...
def test_edit_ad_keeps_the_venue_payment_ids_when_none_are_given(settings) -> None:
    publisher, adapters, _store, _rates = _editor(settings, _live(payment_ids=("7110", "12")))
...
def test_edit_ad_with_a_price_makes_a_floating_ad_fixed(settings) -> None:
    publisher, adapters, _store, _rates = _editor(settings)
...
def test_edit_ad_uses_the_ledger_adv_no_and_leaves_a_different_ledger_ad_alone(settings) -> None:
    ...
    publisher, adapters, _store, _rates = _editor(settings, _live(), other, store=store)
...
def test_edit_ad_switches_an_ad_off_alone(settings) -> None:
    publisher, adapters, store, _rates = _editor(settings)
...
def test_edit_ad_brings_an_offline_ad_online_with_its_changes(settings) -> None:
    publisher, adapters, _store, _rates = _editor(settings, _live(status="offline"))
...
def test_edit_ad_clamps_a_fixed_price_to_a_stored_cap(settings) -> None:
    rates = RateStore()
    rates.set_cap("USD/USDT", "0.95")
    publisher, adapters, _store, _rates = _editor(settings, rates=rates)

    result = publisher.edit_ad("Binance#1", USD_USDT, "777", price="0.99")

    assert result.price == Decimal("0.95")
    assert adapters["binance"].specs[-1].price == Decimal("0.95")
...
def test_edit_ad_refuses_a_floating_ratio_estimated_above_the_cap(settings) -> None:
    rates = RateStore()
    rates.set_cap("USD/USDT", "0.90")
    publisher, adapters, _store, _rates = _editor(settings, rates=rates)

    result = publisher.edit_ad("Binance#1", USD_USDT, "777", price_floating_ratio="91")

    assert result.status == "error"
    assert result.error == (
        "floating ratio 91 would price the ad at about 0.9100, above the USD/USDT cap 0.90"
    )
    assert adapters["binance"].sent == []


def test_edit_ad_establishes_a_required_session_once_before_the_update(settings) -> None:
    publisher, adapters, _store, _rates = _editor(settings, login_request="login-request")
...
def test_edit_ad_reports_a_failing_session_without_updating(settings) -> None:
    publisher, adapters, store, _rates = _editor(
...
def test_edit_ad_dry_run_reads_but_sends_and_stores_nothing(settings, tmp_path: Path) -> None:
    publisher, adapters, store, _rates = _editor(settings, store=AdStore(path=tmp_path / "ads.json"))
...
def test_edit_ad_reports_accounts_adapters_ledger_gaps_and_venue_errors(settings) -> None:
    publisher, _adapters, _store, _rates = _editor(settings)
    no_adapter, _a, _s, _r = _publisher(settings, adapters={})
    unreadable, _a2, _s2, _r2 = _editor(settings, list_error=TransportError("down"))
    failing, _a3, _s3, _r3 = _editor(settings, error=TransportError("boom"))
...
def test_a_disabled_exchange_is_never_listed(okx_off) -> None:
    ...
    publisher, _adapters, _store, _rates = _publisher(okx_off, adapters=adapters)
...
def test_a_disabled_exchange_is_never_edited(okx_off) -> None:
    ...
    publisher, _adapters, _store, _rates = _publisher(okx_off, adapters={"okx": okx})
